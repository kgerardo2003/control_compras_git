...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    const { testRecipient, senderName } = body;
    const recipient = normalizeEmail(testRecipient || body.userEmail);

    if (!recipient || !recipient.includes('@')) {
      return res.status(400).json({
        success: false,
        message: 'Debe proporcionar una dirección de correo de destino válida (ej: nombre@gmail.com).'
      });
    }
    ...
    const info = await transporter.sendMail({
      ...
      text: `Verificación exitosa de servicio de correo SMTP de Google para el Sistema de Control de Compras de la Gerencia de Informática del Organismo Judicial de Guatemala (Vercel Serverless Function).\n\nRemitente: ${user}\nDestinatario: ${recipient}\nServidor: ${host}:${port}\nFecha: ${new Date().toLocaleString('es-GT', { timeZone: 'America/Guatemala' })}`,
      html: `
        <div style="font-family: Arial, Helvetica, sans-serif; max-width: 600px; margin: 0 auto; background-color: #ffffff; border: 1px solid #e2e8f0; border-radius: 12px; overflow: hidden; box-shadow: 0 4px 6px rgba(0,0,0,0.05);">
          ...
          <div style="padding: 24px 28px;">
            <div style="display: inline-block; background-color: #ecfdf5; border: 1px solid #a7f3d0; color: #065f46; padding: 6px 14px; border-radius: 20px; font-size: 12px; font-weight: bold; margin-bottom: 16px;">
              ✓ Despliegue en Vercel Validado Exitosamente
            </div>
